    ...
    fn match(b: Block, line: []const u8) ?[]const u8 {
        ...
        return switch (b.tag) {
            ...
            .code_block => code_block: {
                ...
                if (mem.indexOfNone(u8, trimmed, "`") != null or
                    trimmed.len != b.data.code_block.fence_len)
                {
                    const effective_indent = @min(
                        indent,
                        b.data.code_block.indent,
                    );
...
pub fn feedLine(p: *Parser, line: []const u8) Allocator.Error!void {
    ...
    // block start).
    var maybe_block_start =
        if (!in_code_block or first_unmatched + 2 <= p.pending_blocks.items.len)
    ...
    while (maybe_block_start) |block_start| : ({
        maybe_block_start = try p.startBlock(rest_line);
    }) {
