...
pub fn Renderer(comptime Writer: type, comptime Context: type) type {
    return struct {
        renderFn: *const fn (
            ...
            node: Node.Index,
            writer: Writer,
        ) anyerror!void = renderDefault,
        context: Context,

        const Self = @This();

        pub fn render(r: Self, doc: Document, writer: Writer) !void {
        ...
        pub fn renderDefault(
            ...
            switch (doc.nodes.items(.tag)[@intFromEnum(node)]) {
                ...
                .link => {
                    const href = doc.string(data.link.target);
                    const class, const target = if (href[0] == '#')
                        .{ "", "" }
                    else
                        .{ "external", "_blank" };

                    try writer.print(
                        \\<a class="{[1]s}" href="{0}" target="{[2]s}">
                    ,
                        .{ htmlFormatter(href), class, target },
                    );
                ...
                .autolink => {
                    const href = doc.string(data.text.content);
                    const class, const target = if (href[0] == '#')
                        .{ "", "" }
                    else
                        .{ "external", "_blank" };
                    ...
                    try writer.print(
                        \\<a class="{[1]s}" href="{0}" target="{[2]s}">{0}</a>
                    ,
                        .{ htmlFormatter(href), class, target },
                    );
                },
                ...
                .code_span => {
                    ...
                    try writer.print("<code>{}</code>", .{htmlFormatter(content)});
                },
                .text => {
                    const content = doc.string(data.text.content);
                    try writer.print("{}", .{htmlFormatter(content)});
                },
                .line_break => {
...
pub fn htmlFormatter(bytes: []const u8) std.fmt.Formatter(htmlFormat) {
    return .{ .data = bytes };
}
