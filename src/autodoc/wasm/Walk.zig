    ...
    pub const Index = enum(u32) {
        ...
        pub fn categorize_expr(file_index: File.Index, node: Ast.Node.Index) Category {
            ...
            return switch (ast.nodeTag(node)) {
                ...
                .field_access => {
                    const object_node, const field_ident = ast
                        .nodeData(node)
                        .node_and_token;
                    ...
                    switch (categorize_expr(file_index, object_node)) {
                        .alias => |target| {
                            if (target.get().get_child(field_name)) |idx| {
                                return .{ .alias = idx };
                            }
                        },
                        else => {},
                ...
                => {
                    ...
                    if (if_full.ast.else_expr.unwrap()) |else_expr| {
                        const then_cat = categorize_expr_deep(
                            file_index,
                        ...
                        if (then_cat == .error_set and else_cat == .error_set) {
                            return .{ .error_set = node };
                        }

                        if (then_cat == .type or else_cat == .type or
                ...
                .ptr_type,
                => {
                    const ptr_decl = ast.fullPtrType(node).?;
                    const child_node = ptr_decl.ast.child_type;
                    return categorize_expr(file_index, child_node);
                },
        ...
        fn categorize_call(
            ...
            return switch (file_index.categorize_expr(call.ast.fn_expr)) {
                .type_function => @panic("Unexpected type_function"),
                .alias => |target| {
                    return categorize_decl_as_callee(target, node, call);
                },
                else => .{ .global_const = node },
            };
        }

        fn categorize_decl_as_callee(
            ...
            return switch (decl_index.get().categorize()) {
                .type_function => |n| {
                    return .{ .type_fn_instance = .{
                        .node = call_node,
                        .type_fn = decl_index.get()
                            .file.get()
                            .node_decls.get(n) orelse .none,
                    } };
                },
                .alias => |target| {
                    return categorize_decl_as_callee(target, call_node, call);
                },
        ...
        fn categorize_builtin_call(
            ...
            if (std.mem.eql(u8, builtin_name, "@import")) {
                ...
                if (std.mem.eql(u8, file_path, "std") or
                    std.mem.eql(u8, file_path, "builtin")) return .{
                    .global_const = node,
                };

                if (modules.get(file_path)) |idx| log.info(
                    "importing module: {s} - {}",
                    .{ file_path, idx },
                );

                if (modules.get(file_path)) |idx| return .{
                    .alias = File.Index.findRootDecl(idx),
                };
                ...
                if (files.getIndex(resolved_path)) |idx| return .{
                    .alias = File.Index.findRootDecl(@enumFromInt(idx)),
                };

                log.debug("import target '{s}' did not resolve to any file", .{resolved_path});
...
pub fn add_file(file_name: []const u8, bytes: anytype) !File.Index {
    const ast = try parse(file_name, bytes);
    assert(ast.errors.len == 0);

    const gop = try files.getOrPut(gpa, file_name);
    const file_index: File.Index = @enumFromInt(gop.index);

    if (gop.found_existing) return file_index;

    gop.value_ptr.* = .{ .ast = ast };
...
fn parse(file_name: []const u8, source: anytype) Oom!Ast {
    ...
    const adjusted_source: [:0]const u8 = switch (@TypeOf(source)) {
        [:0]const u8 => source,
        []u8 => s: {
            if (source.len == 0)
                break :s "";
        ...
        else => @compileError("String expected"),
    };
...
fn fn_decl(
    ...
    for (full.ast.params) |param| {
        try w.expr(scope, parent_decl, param);
    }
    try w.expr(scope, parent_decl, full.ast.return_type.unwrap().?);
    try w.maybe_expr(scope, parent_decl, full.ast.align_expr);
    try w.maybe_expr(scope, parent_decl, full.ast.addrspace_expr);
    try w.maybe_expr(scope, parent_decl, full.ast.section_expr);
    try w.maybe_expr(scope, parent_decl, full.ast.callconv_expr);
    try w.maybe_expr(scope, parent_decl, body);
}
...
fn maybe_expr(
    ...
    if (node.unwrap()) |n| return w.expr(scope, parent_decl, n);
}
...
fn expr(
    ...
    switch (ast.nodeTag(node)) {
        ...
        => {
            ...
            try w.expr(scope, parent_decl, lhs);
            try w.expr(scope, parent_decl, rhs);
        },

        .assign_destructure => {
            const full = ast.assignDestructure(node);
            for (full.ast.variables) |variable_node| try w.expr(scope, parent_decl, variable_node);
            _ = try w.expr(scope, parent_decl, full.ast.value_expr);
        },
        ...
        .unwrap_optional,
        .grouped_expression,
        => try w.expr(scope, parent_decl, ast.nodeData(node).node_and_token[0]),

        .@"return" => try w.maybe_expr(scope, parent_decl, ast.nodeData(node).opt_node),

        .anyframe_type => try w.expr(scope, parent_decl, ast.nodeData(node).token_and_node[1]),
        .@"break" => try w.maybe_expr(scope, parent_decl, ast.nodeData(node).opt_token_and_opt_node[1]),
        ...
        .field_access => {
            ...
            try w.expr(scope, parent_decl, object_node);
        },
        ...
        => {
            var buf: [2]Ast.Node.Index = undefined;
            const params = ast.builtinCallParams(&buf, node).?;
            return w.builtin_call(scope, parent_decl, node, params);
        },
        ...
        => {
            ...
            try w.expr(scope, parent_decl, full.ast.fn_expr);
            for (full.ast.params) |param| {
                try w.expr(scope, parent_decl, param);
            }
        },
        ...
        => {
            ...
            try w.expr(scope, parent_decl, full.ast.cond_expr);
            try w.expr(scope, parent_decl, full.ast.then_expr);
            try w.maybe_expr(scope, parent_decl, full.ast.else_expr);
        },
        ...
        .@"while",
        => {
            try w.while_expr(scope, parent_decl, ast.fullWhile(node).?);
        },
        ...
        .for_simple, .@"for" => {
            ...
            for (full.ast.inputs) |input| {
                if (ast.nodeTag(input) == .for_range) {
                    ...
                    try w.expr(scope, parent_decl, start);
                    try w.maybe_expr(scope, parent_decl, end);
                } else {
                    try w.expr(scope, parent_decl, input);
                }
            }
            try w.expr(scope, parent_decl, full.ast.then_expr);
            try w.maybe_expr(scope, parent_decl, full.ast.else_expr);
        ...
        .slice => return w.slice(scope, parent_decl, ast.slice(node)),
        .slice_open => return w.slice(scope, parent_decl, ast.sliceOpen(node)),
        .slice_sentinel => return w.slice(scope, parent_decl, ast.sliceSentinel(node)),
        ...
        => {
            ...
            return w.block(scope, parent_decl, statements);
        },
        ...
        => {
            ...
            try w.maybe_expr(scope, parent_decl, full.ast.align_node);
            try w.maybe_expr(scope, parent_decl, full.ast.addrspace_node);
            try w.maybe_expr(scope, parent_decl, full.ast.sentinel);
            try w.maybe_expr(scope, parent_decl, full.ast.bit_range_start);
            try w.maybe_expr(scope, parent_decl, full.ast.bit_range_end);
            try w.expr(scope, parent_decl, full.ast.child_type);
        },
        ...
        .array_type_sentinel => {
            ...
            try w.expr(scope, parent_decl, len_expr);
            try w.expr(scope, parent_decl, extra.elem_type);
            try w.expr(scope, parent_decl, extra.sentinel);
        },
        .@"switch", .switch_comma => {
            const full = ast.fullSwitch(node).?;
            try w.expr(scope, parent_decl, full.ast.condition);
            for (full.ast.cases) |case_node| {
                const case = ast.fullSwitchCase(case_node).?;
                for (case.ast.values) |value_node| {
                    try w.expr(scope, parent_decl, value_node);
                }
                try w.expr(scope, parent_decl, case.ast.target_expr);
            }
        },
        ...
        => {
            ...
            for (full.ast.elements) |elem| {
                try w.expr(scope, parent_decl, elem);
            }
        },
        ...
        => {
            ...
            for (full.ast.fields) |field| {
                try w.expr(scope, parent_decl, field);
            }
        },
        ...
        => {
            var buf: [1]Ast.Node.Index = undefined;
            return w.fn_decl(scope, parent_decl, .none, ast.fullFnProto(&buf, node).?);
        },
    }
}

fn slice(w: *Walk, scope: *Scope, parent_decl: Decl.Index, full: Ast.full.Slice) Oom!void {
    try w.expr(scope, parent_decl, full.ast.sliced);
    try w.expr(scope, parent_decl, full.ast.start);
    try w.maybe_expr(scope, parent_decl, full.ast.end);
    try w.maybe_expr(scope, parent_decl, full.ast.sentinel);
}
...
fn builtin_call(
    ...
    for (params) |param| try w.expr(scope, parent_decl, param);
}
    ...
    for (statements) |node| {
        switch (ast.nodeTag(node)) {
            ...
            => {
                ...
                try w.global_var_decl(scope, parent_decl, full);
            ...
            .grouped_expression => try w.expr(scope, parent_decl, ast.nodeData(node).node_and_token[0]),

            .@"defer" => try w.expr(scope, parent_decl, ast.nodeData(node).node),
            .@"errdefer" => try w.expr(scope, parent_decl, ast.nodeData(node).opt_token_and_node[1]),

            else => try w.expr(scope, parent_decl, node),
        }
    }
}
...
fn while_expr(w: *Walk, scope: *Scope, parent_decl: Decl.Index, full: Ast.full.While) Oom!void {
    try w.expr(scope, parent_decl, full.ast.cond_expr);
    try w.maybe_expr(scope, parent_decl, full.ast.cont_expr);
    try w.expr(scope, parent_decl, full.ast.then_expr);
    try w.maybe_expr(scope, parent_decl, full.ast.else_expr);
}
