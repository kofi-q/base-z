...
pub const RenderSourceOptions = struct {
    ...
    collapse_whitespace: enum { all, empty_lines, none } = .none,
...
pub fn fileSourceHtml(
    ...
    for (
        ...
        if (std.mem.trim(u8, between, " \t\r\n").len > 0) {
            ...
            switch (options.collapse_whitespace) {
                .all => if (out.items.len > 0 and
                    out.items[out.items.len - 1] != ' ')
                    try out.append(gpa, ' '),

                .empty_lines => if (std.mem.trim(u8, between, " \t").len == 0) {
                    var whitespace: u8 = ' ';
                    for (between) |char| switch (char) {
                        '\n' => {
                            whitespace = '\n';
                            if (out.items.len > 0 and
                                out.items[out.items.len - 1] != '\n')
                            {
                                // for (indent) |_| try out.append(gpa, ' ');
                                try out.append(gpa, whitespace);
                            }
                            break;
                        },
                        else => {},
                    ...
                    if (out.items.len > 0 and
                        out.items[out.items.len - 1] != whitespace)
                    {
                        // for (indent) |_| try out.append(gpa, ' ');
                        try out.append(gpa, whitespace);
                    }
                } else if (tag == .doc_comment and options.skip_doc_comments) {
                    // Skip
                } else {
                    try appendUnindented(out, between, indent);
                },

                .none => try appendUnindented(out, between, indent),
            }
        }
        if (tag == .eof) break;
            ...
            .identifier => i: {
                if (options.fn_link != .none) {
                    ...
                    if (token_index == fn_token + 1) {
                        ...
                        try link_target.fqn2(options.parent, out);
